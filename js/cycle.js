...
$ .fn.cycle = function (options, arg2) {
	...
	return this.each (function () {
		...
		if (startTime) {
			...
			this.cycleTimeout = setTimeout (função () {ir (els, opts2,0,! opts.backwards);}, startTime);
		}
	});
};
...
function handleArguments (cont, opções, arg2) {
	...
	if (options.constructor == String) {
		...
		case 'stop':
			...
			if (! opts)
				retorna falso;
			cont.cycleStop ++; // callbacks procuram por mudança
...
function buildOptions ($ cont, $ slides, els, opções, o) {
		...
		if ($ el.is ('img')) {
			...
			if (loading) {
				if (os && opts.requeueOnImageNotLoaded && ++ options.requeueAttempts <100) {// controla a contagem de novas tentativas para que não fiquemos em loop para sempre
					log (options.requeueAttempts, '- img slide não carregado, re-slideshow:', this.src, this.cycleW, this.cycleH);
					setTimeout (function () {$ (os, oc) .cycle (opções);}, opts.requeueTimeout);
					requeue = true;
					retorna falso; // quebra cada loop
				}
				outro {
					log ('não pôde determinar o tamanho da imagem:' + this.src, this.cycleW, this.cycleH);
				}
			}
		}
		retorno verdadeiro;
	...
	if (opts.next)
		$ (opts.next) .bind (opts.prevNextEvent, function () {retornar avanço (opts, 1);});
	if (opts.prev)
		$ (opts.prev) .bind (opts.prevNextEvent, função () {return advance (opts, 0);});
	if (opts.pager || opts.pagerAnchorBuilder)
		buildPager (els, opts);
...
function supportMultiTransitions (opts) {
	...
	if (opts.multiFx && opts.randomizeEffects) {
		// munge o array fxs para fazer a seleção de efeitos aleatórios
...
// este é o mecanismo principal fn, ele lida com os timeouts, callbacks e índice de slide mgmt
função ir (els, opts, manual, fwd) {
	...
	function queueNext () {
		...
		else if (opts.continuous && p.cyclePause) // contínua mostra o trabalho de um callback posterior, não a lógica do temporizador
			ms = 10;
		if (ms> 0)
...
// corrigir problemas clearType no ie6 definindo uma cor bg explícita
...
// definições de transição - apenas o fade é definido aqui, o pacote de transição define o resto
...
// substitui estes globalmente se você quiser (todos eles são opcionais)
$ .fn.cycle.defaults = {
    ...
    animOutDelay: 0, // permite atraso antes de as transições de slide atuais
    ...
    backwards: false, // true para iniciar a apresentação de slides no último slide e retroceder pela pilha
    ...
    fx: 'fade', // nome do efeito de transição (ou nomes separados por vírgula, ex: 'fade, scrollUp, shuffle')
    ...
    próximo: null, // element, objeto jQuery ou string seletor jQuery para o elemento usar como acionador de evento para o próximo slide
    ...
    requeueOnImageNotLoaded: true, // requeue a apresentação de slides se os slides da imagem ainda não estiverem carregados
    ...
    skipInitializationCallbacks: false, // definido como true para desativar o primeiro antes / depois do retorno de chamada que ocorre antes de qualquer transição
...
// transições Para salvar o tamanho do arquivo sinta-se livre para remover qualquer um desses
// não precisa.
//
...
$ .fn.cycle.transitions.fadeout = função ($ cont, $ slides, opts) {
	...
	opts.before.push (função (curr, next, opts, w, h, rev) {
		$ (curr) .css ('zIndex', opts.slideCount + (rev! == verdadeiro? 1: 0));
		$ (próximo) .css ('zIndex', opts.slideCount + (rev! == verdadeiro? 0: 1));
	});
...
$ .fn.cycle.transitions.scrollUp = função ($ cont, $ slides, opts) {
